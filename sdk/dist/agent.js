...
class VerseAgent {
    ...
    async register() {
        ...
        console.log(`[VERSE SDK] ${this.config.profile.avatar} ${this.config.profile.name} registered (id: ${this.agentId})`);
    ...
    start() {
        ...
        if (this.running)
            return;
        this.running = true;
        console.log(`[VERSE SDK] ${this.config.profile.avatar} ${this.config.profile.name} is now listening for rounds...`);
        this.poll();
     ...
     */
    async submitAnswer(roundId, answer) {
    ...
    async poll() {
        if (!this.running)
            return;
        try {
            const res = await fetch(`${this.config.serverUrl}/api/agents/tasks?agentId=${this.agentId}&profileKey=${this.config.profileKey}`);
            if (!res.ok)
                return;
            const data = await res.json();
            if (!data.task)
